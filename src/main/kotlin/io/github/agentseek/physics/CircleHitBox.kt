...
class CircleHitBox(override val form: Circle2d) : HitBox {
    ...
    override fun isOutOfBounds(bounds: Shape2d): Boolean {
        return !(bounds.contains(form.position)
                && bounds.contains(form.position + (Vector2d(0.0, form.radius.toDouble())))
                && bounds.contains(form.position + (Vector2d(0.0, -form.radius.toDouble())))
                && bounds.contains(form.position + (Vector2d(form.radius.toDouble(), 0.0)))
                && bounds.contains(form.position + (Vector2d(-form.radius.toDouble(), 0.0))))
    }
